    ...
    class Program
    {
        ...
        static List<string> FoundFiles(string[] result, string pathForSearch)
        {
            ...
            foreach (string folder in result)
            {
                try
                {
                    string[] files = SearchFiles(folder, "*lol*" + "*.txt"); 
        ...
        {
            DriveSearch("H:\\", "H:\\Education\\Laboratory_works\\lab1\\result.txt");
            DriveSearch("C:\\", "H:\\Education\\Laboratory_works\\lab1\\result.txt");
            Console.ReadKey();
        }
    }
}
